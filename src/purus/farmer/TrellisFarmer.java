...
public class TrellisFarmer extends Window implements Runnable {
	...
	public TrellisFarmer(Coord rc1, Coord rc2, BotUtils BotUtils,
			boolean harvest, boolean destroy, boolean replant) {
				...
				for(Gob g : crops) {
					...
					// Wait for harvest menu to appear
					 ...
					 // Select the harvest option
				        ...
				        while(true) {
				        	if(BotUtils.findObjectById(g.id)==null || BotUtils.findObjectById(g.id).getStage()!=stageBefore)
				        		break;
				        	else
		            		 BotUtils.sleep(20);
							 if(stopThread)
				...
				for(Gob g : crops) {
		            ...
		            while (BotUtils.getItemAtHand()==null) {
		                 ...
		                 for (Widget w = inv.child; w != null; w = w.next) {
				                if (w instanceof GItem && seedName.contains(((GItem) w).resname())) {
		                         item = (GItem)w;
		                         break;
		                     	}
		                 }
        ...
        synchronized (ui.sess.glob.oc) {
            for (Gob gob : ui.sess.glob.oc) {
	            	if(gob.rc.x<=bigX && gob.rc.x>=smallX &&
	            		...
	            		cropName.contains(gob.getres().name)) {
        ...
        synchronized (ui.sess.glob.oc) {
            for (Gob gob : ui.sess.glob.oc) {
	            	if(gob.rc.x<=bigX && gob.rc.x>=smallX &&
	            		gob.getres()!=null &&
	            		gob.rc.y<=bigY && gob.rc.y>=smallY && 
	            		gob.getres().name.equals(trellis)) {
	            			gobs.add(gob);
	            	}
            }
        }
